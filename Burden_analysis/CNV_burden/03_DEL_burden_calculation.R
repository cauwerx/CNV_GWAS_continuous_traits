# Calculate each individual's deletion burden
...
# Load all CNVs & retain deletions; "ukb_cnv_global.gz" is the final PennCNV output, with all CNV calls in a linear format
...
print(paste0("Number of loaded deletions: ", nrow(cnvs_all)))
...
fwrite(cnvs, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/temp/DEL_ANNOVAR_input.txt", col.names = F, row.names = F, quote = F, sep = "\t")

# Run ANNOVAR
input <- "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/temp/DEL_ANNOVAR_input.txt"
output <- "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/temp/DEL_ANNOVAR_output"
...
# Load output file
cnvs <- as.data.frame(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/temp/DEL_ANNOVAR_output.variant_function", header = F, select = c(1:5, 8:10), col.names = c("REGION", "GENE", "CHR", "START", "END", "IID", "LENGTH", "CN")))
print(paste0("Number of annotated deletions: ", nrow(cnvs)))

# Filter for deletions affecting the genes (stringent)
regions <- c("exonic", "splicing", "ncRNA", "UTR5", "UTR3")
cnvs <- cnvs[grep(paste(regions, collapse = "|"), cnvs$REGION), ]
print(paste0("Remaining deletions after filtering for gene impact (stringent): ", nrow(cnvs)))
...
# Burden
fwrite(cnv_burden, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/final/DEL_burden.txt.gz", col.names = T, row.names = F, quote = F, sep = "\t")

# Sex differences
fwrite(sex_diff, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/final/sex_difference/DEL_burden_sex_diff.txt", col.names = T, row.names = F, quote = F, sep = "\t")
