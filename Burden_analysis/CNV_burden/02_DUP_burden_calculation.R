# Calculate each individual's duplication burden
...
# Load all CNVs & retain duplications; "ukb_cnv_global.gz" is the final PennCNV output, with all CNV calls in a linear format
...
cnvs_all <- cnvs_all[which(cnvs_all$Copy_Number > 2), ]
print(paste0("Number of loaded duplications: ", nrow(cnvs_all)))
...
cnvs <- cnvs_all[which(cnvs_all$Quality_Score >= 0.5), ]
...
fwrite(cnvs, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/temp/DUP_ANNOVAR_input.txt", col.names = F, row.names = F, quote = F, sep = "\t")

# Run ANNOVAR
input <- "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/temp/DUP_ANNOVAR_input.txt"
output <- "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/temp/DUP_ANNOVAR_output"
...
# Load output file
cnvs <- as.data.frame(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/temp/DUP_ANNOVAR_output.variant_function", header = F, select = c(1:5, 8:10), col.names = c("REGION", "GENE", "CHR", "START", "END", "IID", "LENGTH", "CN")))
print(paste0("Number of annotated duplications: ", nrow(cnvs)))

# Filter for duplications affecting the genes (stringent)
regions <- c("exonic", "splicing", "ncRNA", "UTR5", "UTR3")
cnvs <- cnvs[grep(paste(regions, collapse = "|"), cnvs$REGION), ]
print(paste0("Remaining duplications after filtering for gene impact (stringent): ", nrow(cnvs)))
...
# Burden
fwrite(cnv_burden, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/final/DUP_burden.txt.gz", col.names = T, row.names = F, quote = F, sep = "\t")

# Sex differences
fwrite(sex_diff, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/final/sex_difference/DUP_burden_sex_diff.txt", col.names = T, row.names = F, quote = F, sep = "\t")
