...
# Load deletion burden and select the Mb burden; from Burden_analysis/CNV_burden/03_DEL_burden_calculation.R
...
overlap <- overlap[, c(5:9)] # 71836 rows
overlap <- overlap[!duplicated(overlap), ] # 21159 rows
overlap$TO_RM <- ((overlap$i.END - overlap$i.START) + 1)/1000000
...
for(p in names(pheno)[2:ncol(pheno)]) {
	...
	# Fit a linear regression between phenotype and burden
...
fwrite(results, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/12_burden_analysis/continuous/white_british/deletion_only/data/final/SumStat_CNVR_Mb/SumStat_DEL_burden_CNVR_Mb_continuous_All.txt", col.names = T, row.names = F, quote = F, sep = "\t")
