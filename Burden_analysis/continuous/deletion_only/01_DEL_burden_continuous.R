# Continuous trait association with deletion burden - no correction for CNV-GWAS
...
for(p in pheno_names) {
	...
	for (s in 1:3) {
		...
		# Merge burden and phenotype data into a temporary dataframe
		if (s == 1) {
...
fwrite(results, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/12_burden_analysis/continuous/white_british/deletion_only/data/final/SumStat/SumStat_DEL_burden_continuous.txt", col.names = T, row.names = F, quote = F, sep = "\t")
