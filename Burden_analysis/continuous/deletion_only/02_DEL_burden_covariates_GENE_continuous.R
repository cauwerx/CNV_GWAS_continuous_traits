# Continuous trait association with deletion burden - WITH correction for CNV-GWAS
...
# Load deletion burden and select the gene burden; from Burden_analysis/CNV_burden/03_DEL_burden_calculation.R
cnv_burden <- as.data.frame(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/final/DEL_burden.txt.gz", header = T, select = c(1,2,10))) 
print(paste0("Number of samples with deletion burden: ", nrow(cnv_burden)))
...
# Load CNVR covariates and set duplications to 0; from Burden_analysis/CNVR_covariates/CNVR_covariates.R 
...
# Load deletion CNVRs & remodel; from GWAS/07_CNVR/continuous/deletion_only/extract_CNVR_del.R
cnvr <- as.data.table(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/09_CNVR/continuous/white_british/deletion_only/data/final/GWAS_CNVR_del.txt", header = T, select = c(2,9,10,7), col.names = c("CHR", "START", "END", "PHENO"))) 
...
print(paste0("Number of unique autosomal deletion CNVRs: ", nrow(cnvr)))

# Load all CNVs and retain high confidence autosomal deletions; "ukb_cnv_global.gz" is the final PennCNV output, with all CNV calls in a linear format
...
print(paste0("Number of retained autosomal deletions: ", nrow(cnvs)))
...
# Overlap between deletions and deletion CNVRs
...
for(p in names(pheno)[2:ncol(pheno)]) {
	...
	if (nrow(cnvr[which(cnvr$PHENO == p), ]) > 0) { 
		...
		# Select CNVR-overlapping deletions
		...
		# Run ANNOVAR
		input <- paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/12_burden_analysis/continuous/white_british/deletion_only/data/temp/ANNOVAR/DEL_", p,"_ANNOVAR_input.txt")
		output <- paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/12_burden_analysis/continuous/white_british/deletion_only/data/temp/ANNOVAR/DEL_", p,"_ANNOVAR_output")
		...
		print(paste0("Number of annotated deletions: ", nrow(df_gene)))
		df_gene <- df_gene[grep(paste(regions, collapse = "|"), df_gene$REGION), ]
		print(paste0("Remaining deletions after filtering for gene impact (stringent): ", nrow(df_gene)))
...
fwrite(results, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/12_burden_analysis/continuous/white_british/deletion_only/data/final/SumStat_CNVR_GENE/SumStat_DEL_burden_CNVR_GENE_continuous_All.txt", col.names = T, row.names = F, quote = F, sep = "\t")
