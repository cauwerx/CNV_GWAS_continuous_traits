# Continuous trait association with duplication burden - no correction for CNV-GWAS
...
# Load duplication burden; from Burden_analysis/CNV_burden/02_DUP_burden_calculation.R 
...
print(paste0("Number of samples with DUP burden: ", nrow(cnv_burden)))
...
for(p in pheno_names) {
	...
	for (s in 1:3) {
		...
		# Merge burden and apprropriate phenotype data
		if (s == 1) {
...
fwrite(results, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/12_burden_analysis/continuous/white_british/duplication_only/data/final/SumStat/SumStat_DUP_burden_continuous.txt", col.names = T, row.names = F, quote = F, sep = "\t")
