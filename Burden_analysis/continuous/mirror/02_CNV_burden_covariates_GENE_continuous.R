# Continuous trait association with CNV burden - WITH correction for CNV-GWAS
...
# Load CNV burden and select the gene burden; from Burden_analysis/CNV_burden/01_CNV_burden_calculation.R
cnv_burden <- as.data.frame(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/05_phenotypes/CNV_burden/white_british/data/final/CNV_burden.txt.gz", header = T, select = c(1,2,10))) 
...
# Load CNVR covariates; from Burden_analysis/CNVR_covariates/CNVR_covariates.R 
...
# Load CNVRs & remodel; from GWAS/07_CNVR/continuous/mirror/extract_CNVR_mirror.R
cnvr <- as.data.table(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/09_CNVR/continuous/white_british/mirror/data/final/GWAS_CNVR_mirror.txt", header = T, select = c(2,9,10,7), col.names = c("CHR", "START", "END", "PHENO"))) 
...
print(paste0("Number of unique autosomal mirror CNVRs: ", nrow(cnvr)))

# Load all CNVs and retain high confidence autosomal CNVs; "ukb_cnv_global.gz" is the final PennCNV output, with all CNV calls in a linear format
...
for(p in names(pheno)[2:ncol(pheno)]) {
	...
	if (nrow(cnvr[which(cnvr$PHENO == p), ]) > 0) { 
		...
		# Run ANNOVAR
		input <- paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/12_burden_analysis/continuous/white_british/mirror/data/temp/ANNOVAR/CNV_", p,"_ANNOVAR_input.txt")
		output <- paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/12_burden_analysis/continuous/white_british/mirror/data/temp/ANNOVAR/CNV_", p,"_ANNOVAR_output")
...
fwrite(results, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/12_burden_analysis/continuous/white_british/mirror/data/final/SumStat_CNVR_GENE/SumStat_CNV_burden_CNVR_GENE_continuous_All.txt", col.names = T, row.names = F, quote = F, sep = "\t")
