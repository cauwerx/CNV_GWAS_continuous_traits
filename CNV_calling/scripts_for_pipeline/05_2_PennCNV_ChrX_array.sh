...
# Fixed varaibles: path to the HMM file and GC model used by PennCNVV, as well as a file with the sex of analyzed samples
...
sexfile=/home/cauwerx/scratch/cauwerx/projects/CNV_GWA/UKBB_500K_CNV/eid_info/sexfile_no_redacted3.txt
...
# Output variables: log file, raw CNV calls, merged CNV calls, filttered CNVs, CNV call QC, QC summary
log_file=$(echo "${batch}_chrX_log.txt")
output_file1=$(echo "${batch}_chrX_rawcnv.txt")
output_file2=$(echo "${batch}_chrX_merged_rawcnv.txt")
output_file3=$(echo "${batch}_chrX_filtered_rawcnv.txt")
output_file4=$(echo "${batch}_chrX_qc_cnv.txt")
qc_summary=$(echo "${batch}_chrX_qc_summary.txt")
...
# Run PennCNV to detect chrX CNVs
echo "Starting CNV detection: ${batch}, ChrX"
/scratch/beegfs/chuv_kuta/cauwerx/softwares/PennCNV-1.0.5/detect_cnv.pl -test -confidence -directory ${intensity_dir} -listfile ${intensity_list} -hmmfile ${path_to_hmm} -sexfile ${sexfile} -chrx -pfbfile ${path_to_pfb} -gcmodelfile ${path_to_gc} -logfile ${log_file} -output ${output_file1} 

# Run PennCNV to merge adjacent ChrX CNVs
echo "Starting CNV merge: ${batch}, ChrX"
...
# Filter out copy-neutral LOH CNVs (state4, cn=2)
echo "Starting filtering CNVs: ${batch}, ChrX"
sed '/state4,cn=2/d' ${output_file2} > ${output_file3} 

# Run PennCNV to perform QC on autosomal CNVs (default: -qclrrsd 0.3 -qcbafdrift 0.01 -qcwf 0.05)
echo "Starting CNV QC: ${batch}, ChrX"
/scratch/beegfs/chuv_kuta/cauwerx/softwares/PennCNV-1.0.5/filter_cnv.pl ${output_file3} -chrx -qclogfile ${log_file} -qcsumout ${qc_summary} -output ${output_file4} 
