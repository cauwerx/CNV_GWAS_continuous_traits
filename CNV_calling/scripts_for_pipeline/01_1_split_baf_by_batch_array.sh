...
##### Job ##############
#SBATCH --job-name=split_baf                # Give your job a name to recognize it in queue overview
...
# Path to the zipped BAF files; These are available from the UKBB portal
path_to_Zbaf="/data/chuv/sgg/data/UKBB/baf/_001_ukb_baf_chr${chr}_v2.txt.gz" 
echo "Used zipped BAF file: ${path_to_Zbaf}"
...
# Unzip the BAF file; exclude the 11 redacted3 samples; rename chrXY to chr23
if [ "${chr}" == "XY" ]
then
	unzip_chr=$(echo "${path_to_Zbaf:38:-12}23_temp.txt")
	zcat ${path_to_Zbaf} | awk '{$57327=$61154=$68759=$127547=$249705=$254293=$308492=$387485=$409933=$425326=$451667="";gsub(/ +/," ")}1' > ${unzip_chr}
else
	unzip_chr=$(echo "${path_to_Zbaf:38:-10}_temp.txt")
	zcat ${path_to_Zbaf} | awk '{$57327=$61154=$68759=$127547=$249705=$254293=$308492=$387485=$409933=$425326=$451667="";gsub(/ +/," ")}1' > ${unzip_chr}
fi
