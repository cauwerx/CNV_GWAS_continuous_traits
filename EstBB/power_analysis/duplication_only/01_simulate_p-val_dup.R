...
signal <- args[1]
print(paste0("Simulating duplication signal ", signal))


#################################################
### STEP 1: Load files ##########################

# Load replicated duplication-only trait-CNVR association characteristics; "GWAS_CNVR_DUP.withEstResults.withCounts.txt" contains for each signal, in the following order:
# probe ID, chromosome, position in the UKBB
# effect size, SE, and p-value in the UKBB (see GWAS/05_gwas/continuous/duplication_only)
# phenotype name, phenotype field ID
# start position, end position, length, and number of probes of the associated CNVR as defined in the UKBB (see GWAS/07_CNVR/continuous/duplication_only)
...
associations <- as.data.frame(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/raw/GWAS_CNVR_DUP.withEstResults.withCounts.txt"))
...
# Information on the duplication frequency in the EstBB of the assessed region
freq_est <- frequencies[which(frequencies$probe == rs_est), "FreqDUP"]; print(paste0("Duplication frequency (Est BB): ", freq_est))
...
if (is.na(N_est) | is.na(rs_est)) {print(paste0("No data available for ", pheno, " - chr", chr, ":", pos_uk))
	...
	# If data is available
		...
		# Calculate sigma^2: sigma^2 = SD^2 - q*(1-q)*beta^2; SD^2 = 1 as effects are standardized
		sig2 <- 1 - freq_est * (1- freq_est) * b_uk^2
		print(paste0("Sigma^2: ", sig2))
		...
		for (i in 1:n_sim) {
			...
			df <- matrix(nrow = N_est, ncol = 3, dimnames = list(c(), c("CNV", "error", "Y")))
		
			# Simulate CNVs: CNV ~ Bernoulli(q)
			df[, "CNV"] <- as.integer(rbernoulli(N_est, freq_est))
		
			# Simulate the error: error ~ N(0, sig2)
	...
	fwrite(data.frame(p_val = p_val), paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/power_analysis/duplication_only/data/temp/p_values/", pheno, ".", chr, ":", pos_uk, ".dup.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
