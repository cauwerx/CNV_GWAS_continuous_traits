...
# Load replicated mirror trait-CNVR association characteristics; "GWAS_CNVR_MIR.withEstResults.withCounts.txt" columns are explained in EstBB/power_analysis/mirror/01_simulate_p-val_mirror.R
df <- as.data.frame(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/raw/GWAS_CNVR_MIR.withEstResults.withCounts.txt", 
...
for (i in 1:nrow(df)) {
	...
	p_val <- na.omit(as.data.frame(fread(paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/power_analysis/mirror/data/temp/p_values/", df[i, "PHENO"],".", df[i, "CHR"],":", df[i, "POS_UK"],".mirror.txt"))))
...
fwrite(df, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/power_analysis/mirror/data/final/power_EstBB_mirror.txt", col.names = T, row.names = F, quote = F, sep = "\t")
