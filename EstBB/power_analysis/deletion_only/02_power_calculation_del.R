...
# Load replicated deletion-only trait-CNVR association characteristics; "GWAS_CNVR_DEL.withEstResults.withCounts.txt" columns are explained in EstBB/power_analysis/deletion_only/01_simulate_p-val_del.R
df <- as.data.frame(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/raw/GWAS_CNVR_DEL.withEstResults.withCounts.txt", 
...
for (i in 1:nrow(df)) {
	...
	p_val <- na.omit(as.data.frame(fread(paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/power_analysis/deletion_only/data/temp/p_values/", df[i, "PHENO"],".", df[i, "CHR"],":", df[i, "POS_UK"],".del.txt"))))
...
fwrite(df, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/power_analysis/deletion_only/data/final/power_EstBB_del.txt", col.names = T, row.names = F, quote = F, sep = "\t")
