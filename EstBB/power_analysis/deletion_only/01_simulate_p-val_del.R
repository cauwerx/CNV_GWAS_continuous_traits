...
signal <- args[1]
print(paste0("Simulating deletion signal ", signal))


#################################################
### STEP 1: Load files ##########################

# Load replicated mirror trait-CNVR association characteristics; "GWAS_CNVR_DEL.withEstResults.withCounts.txt" contains for each signal, in the following order:
# probe ID, chromosome, position in the UKBB
# effect size, SE, and p-value in the UKBB (see GWAS/05_gwas/continuous/deletion_only)
# phenotype name, phenotype field ID
# start position, end position, length, and number of probes of the associated CNVR as defined in the UKBB (see GWAS/07_CNVR/continuous/deletion_only)
...
associations <- as.data.frame(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/raw/GWAS_CNVR_DEL.withEstResults.withCounts.txt"))
...
# Information on the deletion frequency in the EstBB of the assessed region
freq_est <- frequencies[which(frequencies$probe == rs_est), "FreqDEL"]; print(paste0("Deletion frequency (Est BB): ", freq_est))
...
if (is.na(N_est) | is.na(rs_est)) {print(paste0("No data available for ", pheno, " - chr", chr, ":", pos_uk))
	...
	fwrite(data.frame(p_val = p_val), paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/13_EstBB/power_analysis/deletion_only/data/temp/p_values/", pheno, ".", chr, ":", pos_uk, ".del.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
