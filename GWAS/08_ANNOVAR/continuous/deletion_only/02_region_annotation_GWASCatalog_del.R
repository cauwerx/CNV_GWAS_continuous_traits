...
# Load CNVRs; "GWAS_CNVR_del_ANNOVAR_input.txt" from GWAS/07_CNVR/continuous/deletion_only/extract_CNVR_del.R contains CNVR boundaries in ANNOVAR input format
cnvr <- as.data.frame(fread("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/09_CNVR/continuous/white_british/deletion_only/data/final/GWAS_CNVR_del_ANNOVAR_input.txt", select = c(1:7), col.names = c("CHR", "START", "END", "REF", "OBS", "PHENO", "ID")))
...
for (i in 1:nrow(cnvr)) {
	...
	# Define paths
	input <- paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/10_ANNOVAR/continuous/white_british/deletion_only/data/temp/GWASCatalog/chr", chr, ":", start, "-", end, "_", pheno, ".input.txt")
	output <- paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/10_ANNOVAR/continuous/white_british/deletion_only/data/temp/GWASCatalog/chr", chr, ":", start, "-", end, "_", pheno)
...
# Save
fwrite(cnvr, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/10_ANNOVAR/continuous/white_british/deletion_only/data/final/region_annotation_GWASCatalog/GWASCatalog_matching_pheno_annotation_del_CNVR.txt", col.names = T, row.names = F, quote = F, sep = "\t")
