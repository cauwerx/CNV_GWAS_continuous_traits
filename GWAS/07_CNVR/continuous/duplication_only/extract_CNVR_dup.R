...
# Load lead signals from SCA for all; from GWAS/06_SCA_continuous/duplication_only/SCA_dup_All.R
files_c <- list.files("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/08_SCA/continuous/white_british/duplication_only/data/temp/summary/All/", recursive = F, full.names = T)
df_c <- as.data.frame(fread(files_c[length(files_c)], header = T, select = c(1:6,8)))

# Load lead signals from SCA for males; from GWAS/06_SCA_continuous/duplication_only/SCA_dup_M.R
files_M <- list.files("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/08_SCA/continuous/white_british/duplication_only/data/temp/summary/M/", recursive = F, full.names = T)
df_M <- as.data.frame(fread(files_M[length(files_M)], header = T, select = c(1:6,8)))
df_M <- df_M[df_M$PHENO %in% c("balding", "facial_hair"),  ]

# Load lead signals from SCA for females; from GWAS/06_SCA_continuous/duplication_only/SCA_dup_F.R
files_F <- list.files("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/08_SCA/continuous/white_british/duplication_only/data/temp/summary/F/", recursive = F, full.names = T)
...
for(i in 1:nrow(unique_combo)) {
	...
	rs_file <- paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/09_CNVR/continuous/white_british/duplication_only/data/temp/", rs)
	fwrite(data.frame(ID = rs), rs_file, col.names = F, row.names = F, quote = F, sep = "\t")
	
	# Run PLINK: detect probes in LD
	input <- paste0("/data/FAC/FBM/DBC/zkutalik/default_sensitive/cauwerx/CNV_GWA/duplication_only/ukb_cnv_bed/ukb_cnv_chr", chr)
...
# Replace chromosoome "XY" by "X"
df$CHR <- sub("XY", "X", df$CHR)

# Save
fwrite(df, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/09_CNVR/continuous/white_british/duplication_only/data/final/GWAS_CNVR_dup.txt", col.names = T, row.names = F, quote = F, sep = "\t")
...
# Save
fwrite(df_ANNOVAR, "/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/09_CNVR/continuous/white_british/duplication_only/data/final/GWAS_CNVR_dup_ANNOVAR_input.txt", col.names = F, row.names = F, quote = F, sep = "\t")
