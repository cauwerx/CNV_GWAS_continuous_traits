...
for (p in pheno_list) {
	...
	files <- mixedsort(list.files(paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/06_gwas/continuous/white_british/duplication_only/data/temp/", sex,"/associations"), pattern = paste0("\\.", p, ".glm.linear"), full.names = T, recursive = F))
	...
	fwrite(merged_gwas, paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/06_gwas/continuous/white_british/duplication_only/data/final/", sex,"/gwas_", p, "_", sex, "_dup.txt.gz"), col.names = T, row.names = T, quote = F, sep = "\t")
	...
	if (nrow(sig) >= 1) {
		...
		fwrite(sig, paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/06_gwas/continuous/white_british/duplication_only/data/final/", sex,"/significant_associations/significant_", p, "_", sex, "_dup.txt.gz"), col.names = T, row.names = T, quote = F, sep = "\t")
...
fwrite(sig_all, paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/06_gwas/continuous/white_british/duplication_only/data/final/", sex,"/significant_associations/00_significant_", sex, "_dup.txt.gz"), col.names = T, row.names = F, quote = F, sep = "\t")
